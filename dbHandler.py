...
def insertData(data):
    ...
    db = pymysql.connect("localhost", "root", "Mukund2831", "cdbd")
    cursor = db.cursor()
    print("database connected")

    query = "INSERT INTO cdbd VALUES(0, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % \
...
def retrieveData(name):
    ...
    db = pymysql.connect("localhost", "root", "Mukund2831", "cdpd")
    cursor = db.cursor()
    print("database connected")

    query = "SELECT * FROM cdbd WHERE name='%s'"%name
